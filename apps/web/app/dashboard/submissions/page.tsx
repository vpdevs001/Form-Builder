...
export default function MySubmissionsPage() {
  ...
  const [submissions, setSubmissions] = useState<
    Awaited<ReturnType<typeof api.submission.getByUser.query>>
  >([]);
        ...
        <div className="space-y-3">
          {submissions.map((submission) => (
            <div key={submission.id} className="rounded-lg border border-primary/10 p-4 bg-card/30">
              <p className="text-sm">
                Form reference: <span className="font-mono">{submission.formId}</span>
              </p>
          ...
          {submissions.length === 0 ? (
            <p className="text-sm text-foreground/60">No submissions tied to your account yet.</p>
          ) : null}
        </div>
      </div>
    </div>
