...
export default function SubmissionDetailPage() {
  ...
  const [submission, setSubmission] = useState<Awaited<
    ReturnType<typeof api.submission.getById.query>
  > | null>(null);
  const [values, setValues] = useState<
    Awaited<ReturnType<typeof api.submissionValue.getBySubmissionId.query>>
  >([]);
  ...
  const addValue = async (event: FormEvent) => {
    ...
    try {
      ...
      setValue("");
      await loadData();
      toast.success("Response detail added");
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-[#060913] text-foreground py-12 px-4">
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="bg-card/30 border border-primary/10 rounded-xl p-6">
          ...
          <p className="text-sm text-foreground/60 mt-1">Overview of the submitted response.</p>
          <p className="text-sm mt-2">
            Response ID: <span className="font-mono">{submission.id}</span>
          </p>
          <p className="text-sm">
            Form ID: <span className="font-mono">{submission.formId}</span>
          </p>
            ...
            {values.map((entry) => (
              <div key={entry.id} className="rounded border border-primary/10 p-2 text-sm">
                <span className="font-mono">
                  {fields.find((field) => field.id === entry.fieldId)?.label || entry.fieldId}
                </span>
                : {entry.value}
              </div>
            ))}
        ...
        <form
          onSubmit={addValue}
          className="bg-card/30 border border-primary/10 rounded-xl p-6 space-y-3"
        >
          <h2 className="text-lg font-semibold">Add a response detail</h2>
          <p className="text-sm text-foreground/60">Add a new response value manually.</p>
          <select
            value={fieldId}
          ...
          />
          <Button type="submit" className="bg-primary" disabled={savingValue}>
            {savingValue ? "Saving..." : "Add value"}
          </Button>
        </form>
      </div>
    </div>
