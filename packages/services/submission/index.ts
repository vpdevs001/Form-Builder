...
class SubmissionService {
  ...
  public async createSubmission(input: CreateSubmissionInput): Promise<SubmissionPublic> {
    try {
      const { formId, values, submittedBy, ipAddress, userAgent, respondentEmail, formPassword } =
        input;
      ...
      const [creator] = await db.select().from(users).where(eq(users.id, form.creatorId));
...
If you need help, contact the form creator at ${creator?.email ?? "their email"}.`,
      ...
      if (emailTasks.length > 0) {
        ...
        results.forEach((result) => {
          if (result.status === "rejected") {
            logger.error("Failed to send notification email", { reason: result.reason });
          }
        });
      }
      ...
      handleServiceError(error, "Failed to create submission");
    }
  }
  ...
  public async getSubmissionById({ id }: GetSubmissionByIdInput): Promise<SubmissionPublic> {
    try {
      ...
      handleServiceError(error, "Failed to get submission by id");
    }
  }
  ...
  public async getSubmissionsByFormId({
    ...
    try {
      ...
      handleServiceError(error, "Failed to get submissions by form id");
    }
  }
  ...
  public async getSubmissionsByUser({
    ...
    try {
      ...
      handleServiceError(error, "Failed to get submissions by user");
    }
  }
  ...
  public async deleteSubmission({ id }: DeleteSubmissionInput): Promise<{ success: boolean }> {
    try {
      ...
      handleServiceError(error, "Failed to delete submission");
    }
  }
}
